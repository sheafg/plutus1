...
const string strMessageMagic = "Plutus Signed Message:\n";
...
{
    // Base reward.
    // First block 200K coins premine.
    int64_t nSubsidy = (nHeight > 1) ? 200 * COIN : 250000 * COIN;

    // no premine.

    // halving reward every 360000 blocks.
    int exp = nHeight / 360000;
    // coin overflow after 30 halvings (2^30).
    nSubsidy = (exp <= 30) ? ((exp >= 1) ? nSubsidy / (2 << (exp - 1)) : nSubsidy) : 0;
