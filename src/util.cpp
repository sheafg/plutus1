...
int nAnonymizePlutusAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Plutus
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Plutus
    // Mac: ~/Library/Application Support/Plutus
    // Unix: ~/.plutus
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Plutus";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "plutus.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "plutusd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong PLTNET will not work properly.");
