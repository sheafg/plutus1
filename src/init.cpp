...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: plutus.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: plutusd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 18121)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 18122)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeplutusamount=<n> " + _("Keep N Plutus anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Plutus is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Plutus is probably already running."), strDataDir));
    ...
    if (fDaemon)
        fprintf(stdout, "Plutus server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Plutus") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Plutus to complete") << "\n";
    ...
    nAnonymizePlutusAmount = GetArg("-anonymizeplutusamount", 0);
    if(nAnonymizePlutusAmount > 999999) nAnonymizePlutusAmount = 999999;
    if(nAnonymizePlutusAmount < 2) nAnonymizePlutusAmount = 2;
    ...
    LogPrintf("Anonymize Plutus Amount %d\n", nAnonymizePlutusAmount);
