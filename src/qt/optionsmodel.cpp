...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizePlutusAmount"))
        settings.setValue("nAnonymizePlutusAmount", 1000);
    nAnonymizePlutusAmount = settings.value("nAnonymizePlutusAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizePlutusAmount"))
        SoftSetArg("-anonymizeplutusamount", settings.value("nAnonymizePlutusAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizePlutusAmount:
            return QVariant(nAnonymizePlutusAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizePlutusAmount:
            nAnonymizePlutusAmount = value.toInt();
            settings.setValue("nAnonymizePlutusAmount", nAnonymizePlutusAmount);
            emit AnonymizePlutusAmountChanged(nAnonymizePlutusAmount);
            break;
        default:
            break;
        }
