...
{
    resize(900, 520);
    setWindowTitle(tr("Plutus") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    setObjectName("plutus");
    setStyleSheet("#plutus { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #dedede, stop: 1 #efefef);  }");
    // Accept D&D of URIs
    setAcceptDrops(true);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Plutus address"));
   ...
   // tabGroup->addAction(TradingAction);
    ...
    // Configuration files.
    showGlobalConfigAction = new QAction(QIcon(":/icons/options"), tr("Open plutus.conf"), this);
   ...
   // connect(TradingAction, SIGNAL(triggered()), this, SLOT(gotoTradingPage()));
    ...
    aboutAction = new QAction(QIcon(":/icons/aboutplutus"), tr("&About Plutus"), this);
    aboutAction->setToolTip(tr("Show information about Plutus"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Plutus"));
    ...
    signMessageAction = new QAction(QIcon(":/icons/signmsg"), tr("Sign &message..."), this);
    verifyMessageAction = new QAction(QIcon(":/icons/verifmsg"), tr("&Verify message..."), this);
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Plutus client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Plutus client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Plutus network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Plutus address or malformed URI parameters."));
    }
