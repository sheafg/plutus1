...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizePlutusAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePlutusAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizePlutusAmount = strAnonymizePlutusAmount.remove(strAnonymizePlutusAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePlutusAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizePlutusAmount*COIN) nMaxToAnonymize = nAnonymizePlutusAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizePlutusAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizePlutusAmount));
        strAnonymizePlutusAmount = strAnonymizePlutusAmount.remove(strAnonymizePlutusAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePlutusAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
